...
- 横幅广告位(Banner)： 广点通Banner2.0类型，穿山甲，变现猫

- 开屏广告位(Splash)：广点通，穿山甲，变现猫

- 原生自渲染广告位(Native):广点通，穿山甲，变现猫
...
在工程的build.gradle中引入这些库，具体配置可以参考BxmAdvanceSdkDemo工程的build.gradle设置。
...
implementation files('libs/bxmsdk.jar')
...
<!--变现猫-->
<activity
    android:name="com.bxm.sdk.BxmActivity"
    android:configChanges="keyboard|keyboardHidden|orientation|screenSize" />
<service android:name="com.bxm.sdk.BxmDownloadService" />
...
在res/xml目录下，新建广点通，变现猫的兼容配置file_path.xml,在文件中添加如下代码:
...
public class BannerActivity extends AppCompatActivity implements AdvanceBannerListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
      ...
      //设置默认打底sdk的配置参数，可以为穿山甲，广点通，变现猫，策略服务器不响应就出打底广告，所有广告位都可以设置,请设置为您自己的sdk参数
...
public class SplashActivity extends AppCompatActivity implements AdvanceSplashListener,WeakHandler.IHandler {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        advanceSplash.setDefaultSdkSupplier(new SdkSupplier("12121x","1212xxxx","xxyyyxxyy",AdvanceConfig.SDK_TAG_BXM));
...
支持广点通，穿山甲，变现猫原生自渲染类型广告位，开发者可以提供自定义的模板对广告素材进行渲染,获得更理想的广告展现形式。
...
支持广点通，穿山甲SDK。激励视频需要预先加载，可以在广告缓存成功后展示。广点通激励视频广告展示有过期时间，请尽快在缓存成功后展示广告。
